...
u = graph.face_five_point_detection_net(net)
...
f = open("MTFL_train_log.txt", "w")
g = open("MTFL_test_log.txt", "w")

with tf.Session() as sess:
    sess.run(tf.initialize_all_variables())
    net.load('/afs/csail.mit.edu/u/k/kocabey/Desktop/caffe-tensorflow-master/mynet.npy', sess)
    while ITERATIONS > 0:
        batch_x,batch_point_x,batch_point_y,batch_existence = utils.get_next_trn_batch_MTFL(train_data)
        ...
        if ITERATIONS % 30 == 0:
            ...
            error = 0; l2 = 0;
            for i in range(10):
                batch_x,batch_point_x,batch_point_y,batch_existence = utils.get_next_trn_batch_MTFL(test_data)
        ...
        if ITERATIONS % 100 == 0:
            saver.save(sess, 'MTFL-high-variance-model')
f.close()
g.close()
