...
train_jpg, train_txt = utils.human_random_slice_directories("train")
test_jpg, test_txt = utils.human_random_slice_directories("test")
train_data = utils.import_human_data(train_jpg,train_txt)
test_data  = utils.import_human_data(test_jpg,test_txt)

x = tf.placeholder(tf.float32, shape = [VGG_utils.BATCH_SIZE,224,224,3])
net = VGG_Classic({'data' : x}, trainable = True)

u = VGG_graph.VGG_human_point_detection_net(net)
...
f = open(root + "Experiments/Results/VGG_human_train_log_conv5_2.txt", "w")
g = open(root + "Experiments/Results/VGG_human_test_log_conv5_2.txt", "w")
...
with tf.Session() as sess:
    ...
    while ITERATIONS > 0:
        batch_x,batch_point_x,batch_point_y,batch_existence = VGG_utils.get_next_trn_batch_human(train_data)
        ...
        if ITERATIONS % 30 == 0:
            ...
            error = 0; l2 = 0;
            for i in range(10):
                batch_x,batch_point_x,batch_point_y,batch_existence = VGG_utils.get_next_trn_batch_human(test_data)
        ...
        if ITERATIONS % 200 == 0:
            saver.save(sess, root + 'Experiments/Models/VGG_human_model_conv5_2')
f.close()
g.close()
