...
u = VGG_graph.VGG_face_caltech_point_detection_net(net)
...
f = open(root + "Experiments/Results/VGG_face_train_log_conv5_5.txt", "a")
g = open(root + "Experiments/Results/VGG_face_test_log_conv5_5.txt", "a")

with tf.Session() as sess:
    sess.run(tf.initialize_all_variables())
    #saver.restore(sess, root + 'Experiments/Models/VGG_face_model_conv5_5')
    net.load(root + "VGG_Face/VGG_Face.npy", sess)
    print "VGG Network has been successfully uploaded!"
    while ITERATIONS > 0:
        batch_x,batch_point_x,batch_point_y = VGG_utils.get_next_trn_caltech_batch_face(train_data)
        ...
        if ITERATIONS % 30 == 0:
            ...
            error = 0; l2 = 0;
            for i in range(10):
                batch_x,batch_point_x,batch_point_y = VGG_utils.get_next_trn_caltech_batch_face(test_data)
        ...
        if ITERATIONS % 200 == 0:
            saver.save(sess, root + 'Experiments/Models/VGG_face_model_conv5_5')
f.close()
g.close()
