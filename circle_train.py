...
all_data = utils.import_circle_data()
...
with tf.Session() as sess:
    ...
    a = True
    while ITERATIONS > 0:
        batch_x,batch_y = utils.get_next_trn_batch_face_conv5_convolution(all_data)
        f_dict = {}
        f_dict[x] = batch_x
        for h in range(1,14):
            for w in range(1,14):
        ...
        if acc > .94:
            print "\nYou were very successful. Congratulations!"
            break

    saver.save(sess, 'circle-model-trainable-convolution-multiply', global_step=0)
